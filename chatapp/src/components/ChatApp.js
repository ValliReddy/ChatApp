...
const ChatApp = () => {
  ...
  useEffect(() => {
    if (currentUser) {
      ...
      // Send the 'join' event when user connects
  ...
  const handleSendMessage = () => {
    if (newMessage.trim()) {
      ...
      socket.current.emit('chat message', message); // Emit the message to the server
      setNewMessage('');
    }
  };
          ...
          <div className="col-lg-12">
            <div className="card chat-app">
              ...
              <div id="plist" className="people-list">
                <ul className="list-unstyled chat-list mt-2 mb-0">
                  {[
                    { name: "User 1", img: "https://bootdey.com/img/Content/avatar/avatar1.png" },
                    { name: "User 2", img: "https://bootdey.com/img/Content/avatar/avatar2.png" },
                  ].map((user, index) => (
                  ...
                  <ul className="list-unstyled">
                    {messages.map((msg, idx) => (
                      <li key={idx} className="clearfix">
                        <div className={`message ${msg.sender === currentUser ? 'my-message' : 'other-message'} float-${msg.sender === currentUser ? 'left' : 'right'}`}>
                          {msg.text}
                        </div>
                        <div className="message-data">
